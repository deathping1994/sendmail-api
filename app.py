...
def register():
    try:
        ...
        mongo.db.registeredapp.insert({"appemail":appdetail['email'],"password":appdetail['password'],
        ...
        TEXT = "hi your app has been registered and your auth token is %s" %(bcrypt.generate_password_hash(data['email']+str(datetime.datetime.now())))
...
@app.route('/mailer/<authtoken>',methods=["GET","POST"])
def send_mail(authtoken):
    try:
        res=mongo.db.registeredapp.find_one({"pwhash":authtoken})
        if res is not None:
            data=request.get_json(force=True)
